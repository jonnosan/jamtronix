...
layout, suggested instrument per voice, M4L device wiring, and the
...
- How OSC parameter routing works.
- How to drop `JtxParameterRouter.amxd` on a track.
- How MPE channels compose with the OSC routing.
...
locally by following them once with Live + Max for Live open.
